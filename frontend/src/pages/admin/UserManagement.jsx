    ...
    return (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex justify-between items-end">
                    ...
                    <div className="px-4 py-2 rounded-xl border border-slate-700 text-xs font-bold uppercase tracking-widest text-slate-400">
                        Directory sync
                    </div>
                </div>

            <div className="bg-[#0f172a] rounded-3xl border border-slate-800 p-6 shadow-2xl space-y-5">
                <div className="flex items-end justify-between gap-4 flex-wrap">
                    <div>
                        <h3 className="text-2xl font-black text-white">Patient Search</h3>
                        <p className="text-sm text-slate-400 mt-1">Search by name, phone, govt ID, or NFC card and open the full patient record.</p>
                    </div>
                </div>
                ...
                <form onSubmit={runPatientSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <input
                        ...
                        className="px-4 py-3 rounded-xl border border-slate-800 bg-slate-950 text-slate-100"
                    />
                    <input
                        name="phone"
                    ...
                    <div className="md:col-span-4 flex gap-3">
                        ...
                        <button
                            ...
                            className="px-5 py-3 rounded-xl border border-slate-700 text-slate-300 font-black uppercase text-xs tracking-widest hover:bg-slate-800"
                        >
                            Reset
                        </button>
                    </div>
                    ...
                    <div className="rounded-2xl border border-slate-800 overflow-hidden">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-900/50 border-b border-slate-800">
                                <tr>
                                    <th className="p-4 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Patient</th>
                                    <th className="p-4 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Phone</th>
                                    <th className="p-4 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Govt ID</th>
                                    <th className="p-4 font-bold text-slate-400 uppercase tracking-widest text-[10px]">NFC</th>
                                    <th className="p-4 font-bold text-slate-400 uppercase tracking-widest text-[10px] text-right">Open</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-800">
                                {patientResults.map((patient) => (
                                    <tr key={patient.id} className="hover:bg-slate-800/30 transition-colors">
                                        <td className="p-4">
                                            <div className="font-bold text-slate-100">{patient.fullName}</div>
                                            <div className="text-xs text-slate-500">{patient.username || 'No username'}</div>
                                        </td>
                                        <td className="p-4 text-slate-300">{patient.phone || '-'}</td>
                                        <td className="p-4 text-slate-300 font-mono text-xs">{patient.govtId || '-'}</td>
                                        <td className="p-4 text-slate-300 font-mono text-xs">{patient.nfcId || '-'}</td>
                                ...
                                {!searchLoading && patientResults.length === 0 && (
                                    <tr>
                                        <td colSpan="5" className="p-4 text-sm text-slate-400">No patients matched your search.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
            ...
            <div className="bg-[#0f172a] rounded-3xl border border-slate-800 overflow-hidden shadow-2xl">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-800">
                        {users.map((u) => (
                            <tr key={u.id || u.username} className="group hover:bg-slate-800/30 transition-colors">
                                ...
                                <td className="p-6 text-slate-400 font-mono text-xs">{u.lastLogin ? new Date(u.lastLogin).toLocaleString() : 'No successful login yet'}</td>
                                <td className="p-6 text-right">
                                    {String(u.role).toLowerCase() === 'patient' ? (
                                        ...
                                        <span className="text-slate-500 font-black uppercase text-[10px] tracking-widest p-2">
                                            Read Only
                                        </span>
                                    )}
                                </td>
                ...
                {!loading && error && (
                    <div className="p-6 border-t border-slate-800 text-sm text-red-400">{error}</div>
                )}
                {!loading && !error && users.length === 0 && (
                    <div className="p-6 border-t border-slate-800 text-sm text-slate-400">No users returned by the backend.</div>
                )}
            </div>

            {(detailsLoading || detailsError || selectedPatient) && (
                <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="w-full max-w-4xl rounded-3xl border border-slate-800 bg-[#0b1220] shadow-2xl overflow-hidden">
                        <div className="flex items-center justify-between px-8 py-6 border-b border-slate-800">
                            <div>
                                <h3 className="text-2xl font-black text-white">Patient Details</h3>
                                <p className="text-sm text-slate-400 mt-1">Stored profile data from MongoDB</p>
                            </div>
                            <button
                                type="button"
                            ...
                            {!detailsLoading && selectedPatient && (
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <InfoCard label="Full Name" value={selectedPatient.fullName} />
                                        <InfoCard label="Username" value={selectedPatient.username} />
                                        <InfoCard label="Patient ID" value={selectedPatient.id} mono />
                                        <InfoCard label="Govt ID" value={selectedPatient.govtId} mono />
                                        <InfoCard label="NFC ID" value={selectedPatient.nfcId} mono />
                                        <InfoCard label="Fingerprint ID" value={selectedPatient.fingerprintId} />
                                        <InfoCard label="Date of Birth" value={selectedPatient.dob ? new Date(selectedPatient.dob).toLocaleDateString() : null} />
                                        <InfoCard label="Age" value={selectedPatient.age} />
                                        <InfoCard label="Gender" value={selectedPatient.gender} />
                                        <InfoCard label="Blood Group" value={selectedPatient.bloodGroup} />
                                        <InfoCard label="Height" value={selectedPatient.heightCm ? `${selectedPatient.heightCm} cm` : null} />
                                        <InfoCard label="Weight" value={selectedPatient.weightKg ? `${selectedPatient.weightKg} kg` : null} />
                                        <InfoCard label="Phone" value={selectedPatient.phone} />
                                        <InfoCard label="Email" value={selectedPatient.email} />
                                        <InfoCard label="Address" value={selectedPatient.address} />
                                    </div>
...
function InfoCard({ label, value, mono = false }) {
    return (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            ...
            <div className={`text-sm font-bold text-slate-100 break-words ${mono ? 'font-mono' : ''}`}>{value || 'Not available'}</div>
        </div>
    );
}
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            ...
            <div className="space-y-2 text-sm text-slate-200">
                {items.map((item, index) => (
                    <div key={`${label}-${index}`} className="rounded-xl bg-slate-950/70 px-3 py-2 border border-slate-800">
                        {item}
                    </div>
                ))}
            </div>
